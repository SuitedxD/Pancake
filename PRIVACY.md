...
- Message content is only retained where necessary for enabled moderation, logging, or evidence features.  
...
- Telemetry and aggregated usage metrics (e.g., feature usage counts) which are aggregated or pseudonymized where feasible.
...
- **IMPORTANT NOTES:** 
Pancake is an independent project and is not affiliated with, endorsed by, or sponsored by Discord Inc.
...
Pancake may process certain user data (such as IDs, usernames, avatars, roles or event metadata) even if you do not directly interact with the bot. This occurs automatically as part of server membership and is necessary for features such as greetings, moderation, logging, or role/permission handling. Such processing is based on legitimate interest in providing the service to the server that has enabled Pancake.

Pancake is designed to process only the minimum amount of personal data reasonably necessary to provide its features.
...
**4.1 Plugins & Plugin Authors**: Plugins may process additional data. Plugin Authors are independently responsible for their plugin's collection and processing practices. Users are responsible for reviewing applicable privacy policies before enabling third-party plugins. 
...
**4.3 Legal requests**: Pancake may disclose data to comply with laws or law-enforcement requests.  
**4.4 Transfers**: Data may be stored or processed in jurisdictions other than the user's. Where applicable, reasonable safeguards will be implemented for cross-border data transfers.
...
## 6. Security Measures
**6.1** Reasonable technical and organizational measures are implemented to protect data (access controls, encrypted transport (TLS where applicable), encryption at rest where supported by the underlying infrastructure, credential rotation).  
**6.2** While Pancake implements reasonable technical and organizational safeguards, no method of electronic storage or transmission can be guaranteed to be completely secure.  
...
**8.3** Deletion requests (subject to exceptions such as legal retention obligations). Requests may be denied where permitted by applicable law.
...
**10.3** If the Service is discontinued without a successor, the Team will make reasonable efforts to securely delete hosted personal data that is no longer required for legal or operational purposes.
